 ...
 * @version: 2014.03.20.
 * 
 *           BitmapUtil have been used to reuse the helper methods for other
 *           components can be eager to get the functions of this class.
 */
public class BitmapUtil {

	private static final String TAG = Properties.PREFIX
			+ BitmapUtil.class.getSimpleName();
	...
	public static class ImagePool {
		...
		/**
		 * Method getBitmap(). It've used to pull the Bitmap Item from this Pool
		 * Storage by checking the key, tagged string indicated to this
		 * BitmapItem.
		 * 
		 * @param key
		 *            The Tagged Key, Indicated to the BitmapItem.
		 * @return Bitmap Type.
		 */
				...
				if (item.key.equals(key)) {
					if (mBitmapItems.remove(key)) {
						...
						Log.d(TAG, String.format(
								"Bitmap(ID:%s) move first position, total(%d)",
								key, mBitmapItems.size()));
		...
		/**
		 * Method putBitmap(). It've used to push the BitmapItem to the Pool
		 * Storage by checking the key, tagged indicator of the BitmapItem.
		 * 
		 * @param key
		 *            The Indicator of BitmapItem.
		 * @param bitmap
		 *            Bitmap Type.
		 */
		public void putBitmap(String key, Bitmap bitmap) {
			BitmapItem item = null;
			for (Iterator<BitmapItem> it = mBitmapItems.iterator(); it
					.hasNext();) {
				item = it.next();
				if (item.key.equals(key)) {
					return;
			...
			if (mBitmapItems.size() >= MAX_ITEM_COUNT) {
				for (int i = 0; i < RECYCLE_ITEM_COUNT; i++) {
					...
					Log.i(TAG, String.format("Bitmap(ID: %s) recycled",
							oldItem.key));
		...
		/**
		 * Method getBitmapSize(). It's used to get the Bitmap's size by 2
		...
		/**
		 * Method getBitmapSize(). It's used to get the Bitmap's size by the
		 * array of bytes data.
		 * 
		 * @param bitmap
		 *            Bitmap Type. The raw file( Array of Byte Type)
		 * @return The Size Type. This type included 2 non scalable values:
		 *         width and height.
		 */
		public static Size getBitmapSize(byte[] bitmap) {
			try {
				...
				options.inJustDecodeBounds = true;
				BitmapFactory
						.decodeByteArray(bitmap, 0, bitmap.length, options);
				return new Size(options.outWidth, options.outHeight);
			} catch (Exception e) {
				return null;
			}
		...
		/**
		 * The class BitmapItem which have used for storing the Bitmap with
		 * tagged text into the Pool of Images. -- Field: Key, the tagged text
		 * which will be clued into the Pool of Images. -- Field: Bitmap, the
		 * real-content of BitmapItem Object...
		 */
