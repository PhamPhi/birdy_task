 ...
 * @since 3/19/2014.
 * 
 *        The abstract controller class. It've used to configure the abstract
 *        controller component which will be used for handling the view, models
 *        and connecting, notify to other components into current application.
 * 
 *        By using this abstract class for Activity controller, we can also
 *        reconfigure again for the Fragment Controller.
 * 
 */
public abstract class ActivityControlBase {
	public static final String TAG = Properties.PREFIX
			+ ActivityControlBase.class.getSimpleName();
	...
	/**
	 * Method handleMessage() used to parse the message data and then driving
	 * the process to own each of branches.
	 * 
	 * @param msg
	 *            Message
	 */
	...
	/**
	 * Method obtainIntent() used to obtain new Intent and adding Action for
	 * this intent object
	 * 
	 * @param msg
	 *            Message
	...
	/**
	 * Method sendMessage() used to send a message with the given content(what)
	...
	protected boolean checkOwner(Message msg) {
		if (msg.arg1 == this.hashCode()) {
			...
			Log.d(TAG, String.format("[PROCESS] checkOwner false(%d:%d)",
