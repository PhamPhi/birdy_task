...
public class BirdyApp extends Application {
    ...
    public List<ActivityHandler> handlerList = new ArrayList<ActivityHandler>(); // Define the list of activity handler
    protected AppMainHandler mainAppHandler;
    // Declaring the global variables for handling all components
    public BirdyData mData = new BirdyData(this); // Define the new BDHelper for building the data
    public DataModel mDataModel = new DataModel();
    public BirdyService mService = null; // Defining the Service components at here,,,
    public BirdySettings mSettings = null;

    ////////////////////// GETTERS AND SETTERS METHODS FOR ACCESSING THE COMPONENTS OF APPLICATION /////////////////////
    ...
    ////////////////////// METHODS FOR ADDING, REMOVING THE HANDLER OF LIST OF ACTIVITIES's HANDLERS////////////////////

    /**
     * Method addHandler() used to add the new ActivityHandler to the current main list of handlers
     * @param inHandler The ActivityHandler
     */
    public void addHandler(ActivityHandler inHandler) {
        Log.d(TAG, String.format("The List of Handlers will be added by %s Handler" + inHandler.toString()));
        handlerList.add(inHandler);
    }

    /**
     * Method addHandlerIfNotExisted() used to add the new handler if it was not existed into the current main list
     * of handlers.
     * @param inHandler The ActivityHandler.
     */
        ...
        try{
            for(ActivityHandler handler: handlerList){
                if(handler.hashCode() == inHandler.hashCode()){
                    // Checking the element of list of handlers, if its hashcode has equaled to the given handler's hashcode
                    // Assigning the brnExisted will be equaled to "true"  and breacking this loop
                    blnExisted = true;
                    break;
                }
            }
            ...
            Log.i(TAG, String.format("addHandlerIfNotExisted() throws " + " Exception: " + ex.getMessage() ));
            ex.printStackTrace();
        }
    }

    /**
     * Method removeAllHandler() used to remove all of handlers into the current main list of Activity's Handler.
     * using it into the case duringwhile handling the activities or other UI components.
     */
    public void removeAllHandler(){
        Log.d(TAG, String.format("Removing all handlers of the current list of ActivityHandler"));
        handlerList.clear();
    }

    /**
     * Method removeHandler() used to remove the given handler from the current main list of handlers.
      * @param inHandler The ActivityHandler.
     */
    public void removeHandler(ActivityHandler inHandler){
        Log.i(TAG, String.format("Removing the %s Handler" + inHandler.toString()));
        handlerList.remove(inHandler);
        Log.d(TAG, String.format("The size of list of handlers after removing %s Handler: %d", inHandler.toString(),
                                                                                    handlerList.size()));
    ...
    class AppMainHandler extends Handler {
        public void handleMessage(Message msg) {
            ...
            // Parse the message's content and driven to call the branch of handlers.
            switch (msg.what){
                case ControlMessage.REQUEST_TO_SERVER_NOTICE_LIST:
                    // TODO adding some blocks of codes at here for handling this situation
                    break;
