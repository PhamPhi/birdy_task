...
public class MainBirdyTaskActivity extends ActivityBase {
	...
	public void onCheckNewVersion() {
		if (VersionManager.isExistedBirdyApp(this.getApplicationContext()))
			return;
		Thread checkVersion = new Thread() {
			public void run() {
				mMainHandler.sendMessage(mMainHandler
						.obtainMessage(MainHandler.REQUEST_NOTICE));
			}
		};
	...
	public void sendRequestNotice() {
		// Sometime, we need adding more extra for checking languages, mcc or
		// GPS at here,.
		...
		intent.putExtra(Action.ACTION_REQUEST_MESSAGE,
				ControlMessage.REQUEST_TO_SERVER_NOTICE_LIST);
		startService(intent);
	}
	...
	public static class MenuAdapter extends ArrayAdapter<MenuDrawerItem> {
		...
		public MenuAdapter(Context context) {
			super(context, R.layout.drawer_navigation_list_item,
					R.id.slider_item_title, new ArrayList<MenuDrawerItem>());
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (view != null) {
				...
				ImageView imageDrawerItem = (ImageView) view
						.findViewById(R.id.slider_item_icon);
				imageDrawerItem.setImageResource(item.getIconRes());
				// Hiding the badger

				view.findViewById(R.id.slider_item_badger).setVisibility(
						View.GONE);
				if (item.isSelected()) {
